...
models = {
    'Nativo',       'k', 0.315, [0.100 0.071 0.054 0.0437 0.023];
    'Protocollo 3', 'r', 0.20, [0.14 0.065 0.05 0.03 0.01];
    'Protocollo 2', [0.0 0.5 0.0], 0.31 , [0.061 0.12 0.10 0.03 0.01];
    'Protocollo 1', [0.0 0.4 1.0], 0.1915 , [0.12 0.065 0.045 0.03 0.02];
    'Protocollo 4', [1.0 0.41 0.71], 0.43 , [0.12 0.09 0.065 0.030 0.015];
};
...
for m = 1:size(models,1)
    ...
    sigma = compute_stress(eps0,epsv,Eeq,Eneq);
...
xlabel('t [s]')
ylabel('P_{norm}')
title('Curve di relax longitudinali')
legend(models(:,1))
...
function sigma = compute_stress(eps0,epsv,Eeq,Eneq)
...
for k=1:length(sigma)
    sigma(k) = Eeq*eps0;
    for i=1:n_ramo
        sigma(k) = sigma(k) + Eneq(i)*(eps0 - epsv(i,k));
    end
end
